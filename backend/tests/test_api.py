...
# ---------------------------------------------------------------------------
# The offline crime pack
# ---------------------------------------------------------------------------


def test_the_pack_covers_every_window(client):
    body = client.get("/crime/pack").json()

    assert body["city"] == "dc"
    assert body["windows"] == [30, 60, 180, 365]
    windows_present = {level["windowDays"] for level in body["levels"]}
    assert windows_present == set(body["windows"])


def test_the_pack_carries_the_frame_its_centres_are_in(client):
    """Without it the client cannot derive corners that tile."""
    from getmehome.cities import DC

    body = client.get("/crime/pack").json()
    assert body["projection"] == DC.projection.to_dict()


def test_the_pack_omits_the_finest_ladder_rungs(client):
    """They are where the cell count explodes and are least useful offline."""
    from getmehome.safety.hexgrid import SIZE_LADDER

    body = client.get("/crime/pack").json()
    assert min(body["radii"]) == 375.0
    assert 165.0 in SIZE_LADDER and 165.0 not in body["radii"]
    # And it says so, rather than letting the grid mysteriously stop refining.
    assert body["finestRadius"] == 375.0


def test_the_pack_does_not_claim_night_only_support(client):
    """Baking it would double the file for one toggle."""
    assert client.get("/crime/pack").json()["nightOnlySupported"] is False


def test_pack_cells_carry_what_the_overlay_draws(client):
    body = client.get("/crime/pack").json()
    cells = [c for level in body["levels"] for c in level["cells"]]
    assert cells

    cell = cells[0]
    for key in ("a", "o", "t", "i", "n", "s", "b"):
        assert key in cell, key
    assert isinstance(cell["b"], list)


def test_a_coarser_level_has_fewer_cells_than_a_finer_one(client):
    body = client.get("/crime/pack").json()
    by_radius = {
        level["radius"]: len(level["cells"])
        for level in body["levels"]
        if level["windowDays"] == 365
    }
    radii = sorted(by_radius)
    assert by_radius[radii[0]] >= by_radius[radii[-1]]


def test_the_pack_is_cached_and_tagged(client):
    """Rebuilding it per request would be sixteen passes over the incidents."""
    first = client.get("/crime/pack")
    second = client.get("/crime/pack")
...
def test_a_city_with_no_crime_points_says_how_to_get_them(client):
    response = client.get("/crime/pack", params={"city": "nyc"})
    # NYC is not built in the test environment, so this is the build message.
    assert response.status_code == 503
    assert "nyc" in response.json()["detail"]


def test_the_pack_and_the_live_grid_agree(client):
    """The offline overlay must be the online one, not a second opinion.

    Re-implementing binning on the phone would mean two answers that drift
    apart; shipping finished cells means there is only ever one.
    """
    body = client.get("/crime/pack").json()
    level = next(
        lv for lv in body["levels"] if lv["windowDays"] == 365 and lv["radius"] == 850.0
    )

    live = client.get(
        "/crime/grid",
        params={
            "minLat": 38.78, "minLon": -77.14,
            "maxLat": 39.01, "maxLon": -76.89,
            "windowDays": 365,
        },
    ).json()

    packed_centres = {(c["a"], c["o"]) for c in level["cells"]}
    # The live call picks its own radius for this viewport, so compare totals
    # rather than cells: the same incidents have to be accounted for.
    assert sum(c["t"] for c in level["cells"]) == live["totalIncidents"]
    assert packed_centres


def test_a_packed_cell_rounds_its_coordinates_but_stays_placeable(client):
    """Five decimal places is about a metre — far below a hexagon's size."""
    body = client.get("/crime/pack").json()
    cell = next(c for lv in body["levels"] for c in lv["cells"])
